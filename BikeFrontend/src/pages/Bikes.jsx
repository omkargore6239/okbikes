...
const Bikes = () => {
  ...
  useEffect(() => {
    const fetchBikes=async()=>{
      setLoading(true);
      try{
        const response = await axios.get("http://localhost:8080/api/bikes/list");
        setData(response.data);
        setStatuses(response.data.map((item)=>({id:item.id, status:"Active"})));
      }catch(error){
  ...
  const handleAddBike=(e)=>{
    ...
    axios.post("http://localhost:8080/api/bikes/save", formData)
    .then((response)=> {
  ...
  const handleSaveEdit = (e) =>{
    ...
    axios.put(`http://localhost:8080/api/Bikes/${editingId}`,formData)
    .then((response) =>{
      setData(
                    ...
                    <tr key={bike.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className='px-6 py-4'>
                        ...
                        className={`px-2 py-1 rounded ${
                          statuses.find((row) => row.id === bike.id)?.status ==="Active"
                          ? "bg-green-600 hover:bg-green-600 text-white" :"bg-red-700 hover:bg-red-600 text-white"}`}
