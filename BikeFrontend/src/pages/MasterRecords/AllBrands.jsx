...
const AllBrands = () => {
    ...
    const fetchBrands = async () => {
      ...
      try {
        const response = await axios.get(
          "http://localhost:8080/api/brands/list"
        );
  ...
  const handledAddBrand = (e) => {
    ...
    axios
      .post("http://localhost:8080/api/brands/save", formData)
      .then((response) => {
  ...
  const filteredData = data.filter(
    (item) => item.brandName.toLowerCase().includes(searchQuery.toLowerCase()) // Fix here
  );
